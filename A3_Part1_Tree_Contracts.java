...
return((value == n)
||
(value < n && right!=null? right.member(n):false)
||
(value > n && left!=null?left.member(n):false)
);
}
...
@Requires("member(n) && ((this.value == n && this.left == null && this.right == null) ? this.find(n).get_count()>1: true)")
...
@Ensures("member(n) && (old_n_count+1 == n_count) && result == true") 
public boolean insert(int n) {


if(this.find(n) != null)
n_count = this.find(n).get_count();
else
n_count = 0;
...
// (the pre-condition is from AbsTree.delete)
@Ensures("(member(n) ?((old_n_count == n_count+1) && result == true): result == true) || n_count == 0")
public boolean delete(int n) {
old_n_count = get_count();
 if(super.member(n))
...
@Requires("count > 0 || !stack.isEmpty()")
...
@Requires("true")
@Ensures("node != null?node.value == stack.peek().value:true")
